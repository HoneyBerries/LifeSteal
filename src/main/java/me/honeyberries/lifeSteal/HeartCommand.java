...
public class HeartCommand implements TabExecutor {
        ...
        try {
            if (args.length == 0) {
                ...
                if (player != null) {
                    ...
                    sender.sendMessage(ChatColor.RED + "Invalid username! Player not found.");
                    Bukkit.getLogger().warning("[LifeSteal] Invalid username entered: " + args[2]);
                }
            } else {
            ...
            Bukkit.getLogger().log(Level.SEVERE, "[LifeSteal] Error executing /heart command", e);
        }
        return true;
    }
    ...
    private void handleHealthModification(@NotNull CommandSender sender, @NotNull Player target, @NotNull String[] args) {
        try {
            ...
            Bukkit.getLogger().warning("[LifeSteal] Invalid number format: " + args[1]);
        } catch (Exception e) {
            sender.sendMessage(ChatColor.RED + "An error occurred while modifying health.");
            Bukkit.getLogger().log(Level.SEVERE, "[LifeSteal] Error modifying health", e);
        }
    }
